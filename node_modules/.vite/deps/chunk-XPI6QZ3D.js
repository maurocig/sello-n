...
function await_block(node, get_input, pending_fn, then_fn, catch_fn) {
  ...
  var error_source = runes ? source(void 0) : mutable_source(void 0);
  var resolved = false;
  ...
  block(() => {
    ...
    if (hydrating) {
      ...
      for (var i = 0; i < length; i++) {
        ...
        item = create_item(hydrate_node, state, prev, null, value, key, i, render_fn, flags2);
...
function reconcile(array, state, anchor, render_fn, flags, get_key) {
  ...
  const to_destroy = Array.from(seen2);
  while (current !== null) {
    ...
    effect2 = branch(() => {
      ...
      if (svg || mathml) {
        while (node2.firstChild) {
          anchor.before(node2.firstChild);
        }
      } else {
        anchor.before(node2);
      }
...
function wrap_snippet(component2, fn) {
  return add_snippet_symbol((node, ...args) => {
...
function element(node, get_tag, is_svg, render_fn, get_namespace, location) {
  ...
  var filename = DEV && location && ((_a = current_component_context) == null ? void 0 : _a.function[FILENAME]);
  var tag;
  var current_tag;
  var element2 = null;
    ...
    if (next_tag && next_tag !== current_tag) {
      effect2 = branch(() => {
        ...
        if (render_fn) {
          var child_anchor = (
            /** @type {TemplateNode} */
            hydrating ? element2.firstChild : element2.appendChild(empty())
          );
          if (hydrating) {
        ...
        current_effect.nodes.end = element2;
        anchor.before(element2);
      });
    }
    tag = next_tag;
...
function set_attributes(element2, prev, next2, lowercase_attributes, css_hash) {
  ...
  if (has_hash && !next2.class) {
    next2.class = "";
  }
  var setters = setters_cache.get(element2.nodeName);
  if (!setters) setters_cache.set(element2.nodeName, setters = get_setters(element2));
  var attributes = (
...
function set_dynamic_element_attributes(node, prev, next2, css_hash) {
  if (node.tagName.includes("-")) {
    ...
    for (key in next2) {
      set_custom_element_data(node, key, next2[key]);
    }
    return next2;
  }
...
function animate(element2, options, counterpart, t2, on_finish, on_abort) {
  var is_intro = t2 === 1;
  if (is_function(options)) {
    var a;
    queue_micro_task(() => {
      var o = options({ direction: is_intro ? "in" : "out" });
      a = animate(element2, o, counterpart, t2, on_finish, on_abort);
    });
    return {
      abort: () => a.abort(),
  ...
  if (!(options == null ? void 0 : options.duration)) {
    on_finish == null ? void 0 : on_finish();
    return {
      abort: noop,
      deactivate: noop,
      reset: noop,
...
// node_modules/svelte/src/internal/client/freeze.js
...
//# sourceMappingURL=chunk-XPI6QZ3D.js.map
